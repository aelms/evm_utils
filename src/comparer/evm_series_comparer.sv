 ...
 *    can be implemented using evm_single_line_printer as follows:
 * 
 * \code
 * 
 ...
 *
 */
virtual class evm_series_comparer extends uvm_comparer;
    ...
    extern function              new(string name = "evm_series_comparer");
     ...
     *  
     * Note: Per class description recommendation 1, the evm_series_comparer must be used as the
    ...
    */
     ...
     * 
     * This method should follow the recommendations in the evm_series_comparer description.
     * 
...
endclass: evm_series_comparer

function evm_series_comparer::new(string name = "evm_series_comparer");
    super.new(name);
endfunction: new

function void evm_series_comparer::write_act(uvm_sequence_item si);
    act_q.push_back(si);
    apply_policy(ACTUAL);
endfunction: write_act

function void evm_series_comparer::write_exp(uvm_sequence_item si);
    exp_q.push_back(si);
    apply_policy(EXPECTED);
endfunction: write_exp

function void evm_series_comparer::set_fast_item_compare_mode(boolean_t fast_item_compare_en);
...
function string evm_series_comparer::get_miscompares();
...
function bit evm_series_comparer::compare_field (string name, 
                                                 uvm_bitstream_t lhs, 
                                                 uvm_bitstream_t rhs, 
                                                 int size,
                                                 uvm_radix_enum radix=UVM_NORADIX);
...
function bit evm_series_comparer::compare_field_int (string name, 
                                                     uvm_integral_t lhs, 
                                                     uvm_integral_t rhs, 
                                                     int size,
                                                     uvm_radix_enum radix=UVM_NORADIX); 
...
function bit evm_series_comparer::compare_field_real (string name, 
                                                      real lhs, 
                                                      real rhs);
    if( m_fast_item_compare_en ) begin
        if( lhs != rhs ) begin
...
function bit evm_series_comparer::compare_string (string name,
                                                  string lhs,
                                                  string rhs);
    if( m_fast_item_compare_en ) begin
        if( lhs != rhs ) begin
