 ...
 */
class evm_series_comparer_analysis_imp_if#( type ITEM_T = uvm_sequence_item, type CMP_T = evm_series_comparer ) extends uvm_component;

    typedef evm_series_comparer_analysis_imp_if#(ITEM_T, CMP_T) this_type_t;

    `uvm_object_param_utils( this_type_t );

    `uvm_type_name_decl( $sformatf( "evm_series_comparer_analysis_imp_if(%s, %s)", ITEM_T::type_name(), CMP_T::type_name() ) )
    ...
    function new(string name = "evm_series_comparer_analysis_imp_if", uvm_component parent = null);
...
endclass: evm_series_comparer_analysis_imp_if
