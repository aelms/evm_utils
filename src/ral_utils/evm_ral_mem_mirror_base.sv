 ...
 * extension (evm_ral_mem_mirror) implement the core functionality.
 * 
 * The core functionality is the same as R/W uvm_reg_field and provided by the callback methods in 
 * evm_ral_mem_mirror.  On frontdoor memory writes, the mirror is updated.  On frontdoor memory reads, the read
 ...
 * However, the evm_ral_mem_bkdr_wrapper allows callbacks to be executed on backdoor accesses if desired.
 * 
 ...
 *   
 */
virtual class evm_ral_mem_mirror_base extends uvm_reg_cbs;
    ...
    function new(string name = "evm_ral_mem_mirror_base" );
        super.new(name);
    endfunction: new
...
endclass: evm_ral_mem_mirror_base
