 ...
 * 
 * Extension of the uvm_reg_backdoor that wraps and replaces an original backdoor to allow pre
 * and post access callbacks to be called even on mem.read/write with path of UVM_BACKDOOR
 *
 ...
 *
 * 1) mem.poke/peek()
 *  1.1) evm_ral_mem_bkdr_wrapper::write/read()
 *      1.1.1) if(m_cbks_en) uvm_reg_cbs::pre_write/read()
 *      1.1.2) m_gen_bkdr sets memory
 *      1.1.3) if(m_cbks_en) uvm_reg_cbs::post_write/read()
 *
 * If uvm_mem::read() OR uvm_mem::write() with path=UVM_BACKDOOR are used to access memory, the 
 * appropriate uvm_reg_cbs methods are called by default.  
 *
 * 1) mem.write/read( .path(UVM_BACKDOOR) )
 *    1.1) uvm_reg_cbs::pre_write/read() callbacks
 *    1.2) evm_ral_mem_bkdr_wrapper::write/read()
 ...
 * as needed.
 */
class evm_ral_mem_bkdr_wrapper extends uvm_reg_backdoor;

    `uvm_object_utils(evm_ral_mem_bkdr_wrapper)

    protected uvm_reg_backdoor m_bkdr;             ///< Origional backdoor being wrapped
    protected boolean_t        m_cbks_en = FALSE;  ///< Allow callbacks to be called


    extern         function new(string name="evm_ral_mem_bkdr_wrapper");

    /** \brief Wrap Memory Backdoor
     * Given a memory instance, assigns its backdoor to m_bkdr and replaces it with
     * this class.    
     * 
     * The uvm_post_reset_phase() task is recommended.
     */
    extern virtual function void wrap_mem_backdoor(uvm_mem mem);
...
endclass: evm_ral_mem_bkdr_wrapper

function evm_ral_mem_bkdr_wrapper::new(string name="evm_ral_mem_bkdr_wrapper");
    super.new(name);
endfunction: new

function void evm_ral_mem_bkdr_wrapper::wrap_mem_backdoor(uvm_mem mem);
    m_bkdr = mem.get_backdoor();
    mem.set_backdoor(this);
endfunction: wrap_mem_backdoor

function void evm_ral_mem_bkdr_wrapper::set_mem_cbks_en(boolean_t mem_cbks_en);
    m_cbks_en = mem_cbks_en;
endfunction: set_mem_cbks_en

task evm_ral_mem_bkdr_wrapper::write(uvm_reg_item rw);
    uvm_mem mem;
...
task evm_ral_mem_bkdr_wrapper::read(uvm_reg_item rw);
    uvm_mem mem;
    if(!$cast(mem,rw.element)) begin
